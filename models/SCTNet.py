...
class InjectedLinear(nn.Module):
    def __init__(self, in_features, out_features, bias=False, r=1, r2=128):
        super().__init__()

        self.linear_adapter = nn.Linear(in_features, out_features, bias)
        self.adapter_down = nn.Linear(in_features, r, bias=False)
        self.adapter_up = nn.Linear(r, out_features, bias=False)
        self.adapter_down2 = nn.Linear(in_features, r2, bias=False)
        self.adapter_up2 = nn.Linear(r2, out_features, bias=False)
        self.scale1 = 1.0
        self.scale2 = 1.0

        nn.init.normal_(self.adapter_down.weight, std=1 / r**2)
    ...
    def forward(self, input):
        # print(type(self.scale1), type(self.scale2))
        if isinstance(self.scale1, (int, float)):
            return self.linear_adapter(input) + \
...
def inject_trainable_adapter_transformer(
    model: nn.Module,
    target_replace_module: List[str] = ["WindowAttention", ],
    r1: int = 1,
    r2: int = 64,
):
    """
        ...
        if _module.__class__.__name__ in target_replace_module:
            ...
            for name, _child_module in _module.named_modules():
                if _child_module.__class__.__name__ == "Linear":
                    ...
                    _tmp.linear_adapter.weight = weight
                    if bias is not None:
                        _tmp.linear_adapter.bias = bias
...
if __name__ == '__main__':
    ...
    print("--"*10)
    inject_trainable_adapter_transformer(model)
    print(model)
